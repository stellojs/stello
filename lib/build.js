...
module.exports = function(opts, cb) {
  ...
  fs.readFile('./src/index.hbs', function(err, data) {
    ...
    t.getBoardCards(opts.trelloBoardUrl, function(err, cards) {
      ...
      // Don't really want to force the use to install a markdow compiler...
      // just augment card data here?

      async.each(cards,
        function(card, done) {
          fs.writeFile('dist/' + card.pos + '-' + slug(card.name).toLowerCase() + '.html', cardTpl(card), done)
        },
        function(err) {
          cb(err);
        });
