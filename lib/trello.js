...
/**
 * Get board data by id or url
 *
 * @param {String} boardId The id or url for a given board
 * @param {Function} cb The callback
 */
Trello.prototype.getBoard = function(boardId, cb) {
  boardId = getBoardIdFromUrl(boardId);
  var apiKey = this.apiKey
    , authToken = this.authToken
    , uri = format('https://api.trello.com/1/boards/%s?key=%s&token=%s', boardId, apiKey, authToken);
  request(uri, function(error, resp, body) {
    if(error) { return cb(error); }
    var board;
    try {
...
var assert200 = function(resp, body) {
  if(200 !== resp.statusCode) {
    throw new Error('Could not communicate with Trello: ' + body);
  }
};
